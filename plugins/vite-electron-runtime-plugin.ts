          ...
          (err: NodeJS.ErrnoException | null, stats: fs.Stats) => {
            ...
            if (stats.isDirectory()) {
              ...
              fs.watchFile(filePath, (curr: fs.Stats, prev: fs.Stats) =>
                onFileChange(curr, prev, IP, server)
              );
            }
          }
        );
...
const watchPreloadFileChange = (IP: string, server: ViteDevServer) => {
  fs.watchFile("preload/index.ts", (curr: fs.Stats, prev: fs.Stats) =>
    onFileChange(curr, prev, IP, server)
  );
};
  ...
  return {
    ...
    configureServer(server) {
      ...
      server?.httpServer?.once("listening", () => {
        ...
        // 启动 electron 进程
        electronProcess = spawn(require("electron") as any, [
          "dist/main.js",
          IP,
        ]);
